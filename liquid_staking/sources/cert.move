...
module liquid_staking::cert {
    ...
    fun init(witness: CERT, ctx: &mut TxContext) {
        ...
        let (treasury_cap, metadata) = coin::create_currency<CERT>(
            witness, 9, b"ankrSUI", b"Ankr Staked SUI",
            b"Ankr's SUI staking solution provides the best user experience and highest level of safety, combined with an attractive reward mechanism and instant staking liquidity through a bond-like synthetic token called ankrSUI.",
            option::some<Url>(url::new_unsafe_from_bytes(b"https://assets.ankr.com/staking/tokens/ankrSUI.svg")),
            ctx
        );
